...
func play_music(_audio : AudioStream) -> void:
	if _audio == music_players[curr_music_player].stream:
		return
		
	curr_music_player += 1
	if curr_music_player > 0:
		curr_music_player = 0
		
	var current_player : AudioStreamPlayer = music_players[curr_music_player]
	current_player.stream = _audio
	play_and_fade_in(current_player)
	
	var old_player = music_players[1]
	if curr_music_player == 1:
